...
**Intent:** Shield Mode is **hardcoded on** for every property: owners and managers **cannot** turn it off from the product UI, and API responses should show Shield as **ON**. The underlying column and legacy “turn off” code paths remain in the codebase for a future toggle (`SHIELD_MODE_ALWAYS_ON` in `app/services/shield_mode_policy.py`). Status Confirmation (**Dead Man’s Switch** on eligible stays) does **not** depend on flipping a Shield switch off.
