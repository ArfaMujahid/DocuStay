    ...
    if bypass_verification:
        ...
        try:
            ...
            if "email" in msg.lower() or "unique" in msg.lower():
                raise HTTPException(
                    status_code=400,
                    detail=_register_email_taken_message(data.role),
                )
            raise
        db.refresh(user)
        ...
        pending = PendingRegistration(
            ...
            extra_data={"poa_signature_id": data.poa_signature_id} if data.role == UserRole.owner else None,
        )
        db.add(pending)
        db.commit()
        db.refresh(pending)
...
def verify_email(request: Request, data: VerifyEmailRequest, db: Session = Depends(get_db)):
    ...
    if pending:
        if not code or pending.verification_code != code:
            create_log(
                db,
                CATEGORY_FAILED_ATTEMPT,
        ...
        if pending.role == UserRole.owner:
            user = _complete_pending_owner(db, pending)
            send_owner_welcome_email(user.email, user.full_name)
        else:
    ...
    if getattr(user, "email_verified", False):
        token = create_access_token(user.id, user.email, user.role)
        return Token(access_token=token, user=UserResponse.model_validate(user))
    if not code or not user.email_verification_code or user.email_verification_code != code:
        create_log(
            db,
            CATEGORY_FAILED_ATTEMPT,
            "Email verification failed",
