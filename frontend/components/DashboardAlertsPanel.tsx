                ...
                <li key={entry.id} className="border-b border-slate-100 last:border-b-0 px-4 py-3">
                  <div className={`rounded-lg border p-3 ${severityStyles.info}`}>
                    <div className="min-w-0 flex-1">
                      ...
                      {(entry.message !== entry.title || !entry.property_name) && (
                        <p className="text-sm mt-1 text-slate-600">{entry.message}</p>
                      )}
                      {entry.category && (
                      ...
                      <p className="text-xs mt-2 text-slate-500">{formatAlertTime(entry.created_at)}</p>
                    </div>
                  </div>
                </li>
              ))}
