...
import { dashboardApi, authApi, agreementsApi, publicApi, APP_ORIGIN, type GuestStayView, type GuestPendingInviteView, type OwnerAuditLogEntry } from '../../services/api';
...
export const GuestDashboard: React.FC<{ user: UserSession; navigate: (v: string) => void; notify: (t: 'success' | 'error', m: string) => void }> = ({ user, navigate, notify }) => {
  ...
  // Current period (not ended): includes authorized stays before check-in — previously only "checked in" showed as ongoing, so new accepts looked empty.
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (!slug) return;
                      window.open(publicApi.getLivePoaPdfUrl(slug), '_blank', 'noopener,noreferrer');
                    }}
                  >
                    View POA
                  </Button>
