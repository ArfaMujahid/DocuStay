    ...
    <div className="w-full max-w-5xl py-4 md:py-6">
      {!embedded && (
        <button
          ...
          className="flex items-center gap-2 text-white/90 hover:text-white font-medium text-sm uppercase tracking-wider transition-colors mb-6"
        >
      ...
      <header className="mb-6">
        ...
        <p className="text-white/90 text-sm mt-1">Manage your DocuStay account and preferences.</p>
      </header>

      <div className="space-y-8">
        {/* Personal Information – cosmic card */}
        <Card className="p-8 md:p-10 border-white/10">
          <h2 className="text-lg font-semibold text-white border-b border-white/10 pb-3 mb-8">Personal Information</h2>
        ...
        {isOwner && onContextModeChange && (
          <Card className="p-8 md:p-10 border-white/10">
            <h2 className="text-lg font-semibold text-white border-b border-white/10 pb-3 mb-6">Mode</h2>
            <p className="text-white/90 text-sm mb-4">
              <strong className="text-white">Personal mode:</strong> Manage your own properties and statuses—multiple properties, primary residence, occupancy (vacant, away, inactive). Use resident features (presence, guest invites) for units where you live.
            </p>
            <p className="text-white/90 text-sm mb-4">
              <strong className="text-white">Business mode:</strong> Management scope only. Property status, occupancy, Shield Mode, billing, and event ledger. No personal guest or stay activity is shown.
            </p>
            <ModeSwitcher
        ...
        {isOwner && contextMode !== 'personal' && (
          <Card className="p-8 md:p-10 border-white/10">
            <h2 className="text-lg font-semibold text-white border-b border-white/10 pb-3 mb-6">Subscription & Billing</h2>
            {billingLoading ? (
              <p className="text-white/90 text-sm">Loading…</p>
            ) : billing === null ? (
              <p className="text-white/90 text-sm">Unable to load billing information. Try the Billing tab or refresh.</p>
            ) : (
              <div className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                    <p className="text-xs font-medium text-white/70 uppercase tracking-wider mb-1">Active units (properties)</p>
                    <p className="text-2xl font-bold text-white">{billing.current_unit_count ?? 0}</p>
                    <p className="text-sm text-white/80 mt-1">$1/unit/mo baseline</p>
                  </div>
                  <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                    <p className="text-xs font-medium text-white/70 uppercase tracking-wider mb-1">Units with Shield</p>
                    <p className="text-2xl font-bold text-white">{billing.current_shield_count ?? 0}</p>
                    <p className="text-sm text-white/80 mt-1">$10/unit/mo when active</p>
                  </div>
                </div>
                <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                  <p className="text-sm font-medium text-white mb-1">Monthly subscription</p>
                ...
                {(billing.current_unit_count ?? 0) === 0 && (
                  <p className="text-white/80 text-sm">Add a property to start your subscription.</p>
                )}
                {billing.can_invite === false && (
                  <div className="p-4 rounded-lg bg-amber-500/10 border border-amber-400/30">
                    <p className="text-sm font-medium text-amber-200">Pay your onboarding invoice to invite guests.</p>
                    <p className="text-xs text-amber-200/90 mt-1">Go to Billing to view and pay.</p>
                  </div>
                )}
        ...
        {isOwner && (
          <Card className="p-8 md:p-10 border-white/10">
            <h2 className="text-lg font-semibold text-white border-b border-white/10 pb-3 mb-6">Portfolio</h2>
            <p className="text-white/90 text-sm mb-4">
            ...
            {portfolioLoading ? (
              ...
              <p className="text-white/80 text-sm">Unable to load portfolio link. Try refreshing.</p>
            )}
          </Card>
        )}
