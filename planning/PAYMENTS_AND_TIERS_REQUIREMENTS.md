...
| **`app/services/stay_timer.py`** | Sets `prop.shield_mode_enabled = 1` on last day of stay and when DMS triggers (UNCONFIRMED). | After setting `shield_mode_enabled = 1`, call billing service to sync Shield quantity (or run sync at end of job for all affected owners). |
