...
| **Shield ON** (DMS triggered, no owner response) | `app/services/stay_timer.py` – sets `shield_mode_enabled = 1` | Yes, same flow (~378) |
...
1. **Shield ON (e.g. last day of stay or DMS)**  
   - Have a property with an active stay where Shield gets turned **on** (e.g. last day of stay job or DMS trigger).  
...
| **3. Shield billing** | Turn Shield off; confirm vacated when Shield was on; let Shield turn on (last day/DMS) | Stripe: Shield quantity changes; proration on next invoice/activity |
