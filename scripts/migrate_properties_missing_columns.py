"""
Add any missing columns to the properties table (tax_id, apn, vacant_monitoring_*).
Run on production if you see: column "tax_id" / "apn" / "vacant_monitoring_*" does not exist.

Run from project root: python scripts/migrate_properties_missing_columns.py
"""
import os
import sys
...
def main():
    ...
    # (column_name, PostgreSQL type, SQLite type for fallback)
    columns = [
        ("tax_id", "VARCHAR(64)", "VARCHAR(64)"),
        ("apn", "VARCHAR(64)", "VARCHAR(64)"),
    ...
    with engine.connect() as conn:
        for col_name, pg_type, sqlite_type in columns:
            try:
                ...
                conn.commit()
                print(f"properties.{col_name}: added")
            except Exception as e:
                err = str(e).lower()
                if "already exists" in err or "duplicate" in err:
                    ...
                    try:
                        ...
                        conn.commit()
                        print(f"properties.{col_name}: added (SQLite)")
                        ...
                        if "duplicate column" in str(e2).lower() or "already exists" in str(e2).lower():
                            ...
                            raise
    print("Done. Restart the app if it was failing on properties queries.")
