"""
Create a manual one-off Stripe invoice that looks like the monthly subscription
(baseline $1/unit + Shield $10/unit) for a given owner. Use this to test whether
such an invoice stays open/paid or becomes void.
...
def main():
    ...
    try:
        ...
        # One-off invoice (not tied to a subscription), same collection method as onboarding
        invoice = stripe.Invoice.create(
            ...
            description="DocuStay monthly subscription (manual test)",
        )
        # Baseline: $1/unit
        stripe.InvoiceItem.create(
            ...
            amount=100 * units,
            currency="usd",
            description=f"DocuStay Baseline (per unit) × {units}",
        )
        if shield_units > 0:
            stripe.InvoiceItem.create(
                customer=profile.stripe_customer_id,
                invoice=invoice.id,
                amount=1000 * shield_units,
                currency="usd",
                description=f"DocuStay Shield (per unit) × {shield_units}",
            )
        ...
        print(f"  Units: {units}, Shield: {shield_units}. Amount: ${(100 * units + 1000 * shield_units) / 100:.2f} USD.")
