...
from app.seed import _SECTION_3_CLAUSES
...
def _make_jinfo(
    ...
    *,
    section_3_clause: str | None = None,
) -> JurisdictionInfo:
    ...
    return JurisdictionInfo(
        ...
        section_3_clause=section_3_clause,
...
class TestGuestAcknowledgmentBuilder(unittest.TestCase):
    ...
    def test_california_section3_and_disclaimer(self) -> None:
        jinfo = _make_jinfo("CA", "California", "Cal. Civ. Code § 1940", section_3_clause=_SECTION_3_CLAUSES["CA"])
        ...
        self.assertIn("Platform Role", content)

    def test_florida_section3_and_disclaimer(self) -> None:
        jinfo = _make_jinfo("FL", "Florida", "Florida Statutes § 82.036", section_3_clause=_SECTION_3_CLAUSES["FL"])
        content = _build_guest_acknowledgment(
            "FL", jinfo,
        ...
        self.assertIn("revocable license to occupy and not a lease", content)
        self.assertIn("not a legal determination", content)
        self.assertIn("Florida Residential Landlord and Tenant Act", content)
        self.assertIn("Platform Role", content)

    def test_new_york_section3_and_disclaimer(self) -> None:
        jinfo = _make_jinfo("NYC", "New York", "NYC Admin Code § 26-521", section_3_clause=_SECTION_3_CLAUSES["NY"])
        content = _build_guest_acknowledgment(
            "NYC", jinfo,
        ...
        self.assertIn("thirty (30) days or more", content)
        self.assertIn("New York Real Property Law", content)
        self.assertIn("Platform Role", content)

    def test_generic_section3_for_tx(self) -> None:
        jinfo = _make_jinfo("TX", "Texas", "Texas Property Code § 92.001", section_3_clause=_SECTION_3_CLAUSES["TX"])
        content = _build_guest_acknowledgment(
            "TX", jinfo,
        ...
        self.assertIn("Texas Property Code Section 92.001", content)
        self.assertIn("not a legal determination", content)
    ...
    def test_pdf_generates_with_bold(self) -> None:
        ...
        content = "**Property:** 123 Main St\n**Guest:** [Guest Name]\n**Acknowledgment of Authority:** Some text."
